...
import json import os import types from functools import lru_cache from pathlib import Path from typing import Any, Callable, Dict, Generator, Iterable, List, Mapping, MutableMapping

import tomllib  # Python ≥3.11 (fallback provided below)

import yaml  # PyYAML from pydantic import BaseModel, Field, PositiveInt, SecretStr, field_validator, model_validator from pydantic_settings import BaseSettings, SettingsConfigDict
...
DEFAULT_SETTINGS_FILE = "default.toml" EXTERNAL_SETTINGS_FILE_NAMES = ("settings.toml", "settings.yaml", "settings.yml") ENV_SETTINGS_PATH_KEY = "AI_MEMORY_SETTINGS"  # path override

LogLevel = str  # for readability; actual validation done later

def _toml_loader(path: Path) -> Dict[str, Any]: if not path.exists(): return {} return tomllib.loads(path.read_text())

def _yaml_loader(path: Path) -> Dict[str, Any]: if not path.exists(): return {} return yaml.safe_load(path.read_text()) or {}
...
class Settings(BaseSettings): """Runtime settings for the whole service."""
...
@lru_cache(maxsize=1) def get_settings() -> Settings:  # noqa: D401 – short docstring is fine """Return cached settings instance (singleton)."""

return Settings()  # type: ignore[arg-type]

Convenience – configure logging right away when imported as early as possible

if "LOG_CFG_INITIALISED" not in os.environ: import logging.config from importlib.resources import files
