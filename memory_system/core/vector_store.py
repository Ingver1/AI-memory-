...
The binary **blob** (``*.bin``) keeps raw ``float32`` vectors, while an
SQLite side‑table (``*.db``) stores *offset/length/hash* metadata.  All heavy
blocking I/O is off‑loaded to the default thread‑pool so the event‑loop stays
responsive.
"""
...
_PAGE_SIZE: int = os.sysconf("SC_PAGESIZE") if hasattr(os, "sysconf") else 4096
_FLOAT_SIZE: int = 4  # bytes per float32


class StorageError(Exception):
    """Raised on any persistence failure (I/O, corruption)."""


class ValidationError(Exception):
    """Raised when user input is invalid (wrong dtype/dim/etc.)."""


class _Meta:
    """In‑memory metadata for a single vector."""
...
class AbstractVectorStore(ABC):
    """Minimal async contract for vector persistence.
...
class VectorStore(AbstractVectorStore):  # noqa: D101 – top‑level docstring above
    """Durable **on‑disk** vector store (async).
