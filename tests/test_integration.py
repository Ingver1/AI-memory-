...
from memory_system.utils.security import EnhancedPIIFilter, EncryptionManager
...
class TestEndToEndMemoryWorkflow:
    ...
    async def test_complete_memory_lifecycle(self, full_system):
        ...
        # Test data
        memories = [
            {"id": "mem1", "text": "Machine learning algorithms are powerful tools for data analysis."},
            {"id": "mem2", "text": "Deep learning neural networks can recognize complex patterns."},
            {"id": "mem3", "text": "Natural language processing enables computers to understand human text."},
            {"id": "mem4", "text": "Computer vision allows machines to interpret visual information."},
            {"id": "mem5", "text": "Reinforcement learning helps agents learn through trial and error."},
        ]
        ...
        if result_ids:
            ...
            assert any(keyword in top_memory["text"].lower() 
                      for keyword in ["neural", "deep", "learning"])
    ...
    async def test_memory_search_precision(self, full_system):
        ...
        for domain, query in test_queries.items():
            ...
            correct_domain_results = sum(1 for result_id in result_ids 
                                       if result_id in domain_memories)
...
class TestSecurityIntegration:
    ...
    def test_pii_filtering_integration(self):
        ...
        for text in test_texts:
            ...
            for detection_type, detected_values in detections.items():
